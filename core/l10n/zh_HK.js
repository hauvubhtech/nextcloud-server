OC.L10N.register(
    ...
    "Please make sure to set the \"overwrite.cli.url\" option in your config.php file to the URL that your users mainly use to access this xD Drive. Suggestion: \"{suggestedOverwriteCliURL}\". Otherwise there might be problems with the URL generation via cron. (It is possible though that the suggested URL is not the URL that your users mainly use to access this xD Drive. Best is to double check this in any case.)" : "請確保將 config.php 文件中的“overwrite.cli.url”選項設置為您的用戶主要用於訪問此 xD Drive 的 URL。建議：“{suggestedOverwriteCliURL}”。否則，通過 cron 生成的 URL 可能會出現問題。（但是，建議的 URL 可能不是您的用戶主要用於訪問此 xD Drive 的 URL。最好是仔細檢查以防萬一。）",
    ...
    "The update was unsuccessful. Please report this issue to the <a href=\"https://github.com/nextcloud/server/issues\" target=\"_blank\">xD Drive community</a>." : "更新不成功。請在 <a href=\"https://github.com/nextcloud/server/issues\" target=\"_blank\">xD Drive 社群</a>舉報此問題。",
    ...
    "It looks like you are trying to reinstall your xD Drive. However the file CAN_INSTALL is missing from your config directory. Please create the file CAN_INSTALL in your config folder to continue." : "您似乎正在嘗試重新安裝您的Nextcloud。然而，檔案 CAN_INSTALL 並未在您的config目錄中。請在您的config目錄中建立 CAN_INSTALL檔以繼續。",
    ...
    "Enter your subscription key to increase the user limit. For more information about xD Drive Enterprise see our website." : "請輸入你獲得的訂閱代碼以解開用戶數量限制。有關Nextcloud企業版的詳細信息請參見我們的網站。",
    ...
    "xD Drive 23 is the last release supporting PHP 7.3. xD Drive 24 requires at least PHP 7.4." : "xD Drive 23 是支援 PHP 7.3 的最後版本，xD Drive 24 僅支援 PHP 7.4 以上的版本。",
    "Some columns in the database are missing a conversion to big int. Due to the fact that changing column types on big tables could take some time they were not changed automatically. By running 'occ db:convert-filecache-bigint' those pending changes could be applied manually. This operation needs to be made while the instance is offline. For further details read {linkstart}the documentation page about this ↗{linkend}." : "數據庫中的某些列缺少對 big int 的轉換。由於更改大表上的列類型可能需要一些時間，因此無法自動更改它們。通過運行 “occ db：convert-filecache-bigint”，可以手動應用這些未決的更改。實例處於脫機狀態時需要執行此操作。有關更多詳細信息，請{linklink}關於此↗{linkend}的文檔頁面。",
    "To migrate to another database use the command line tool: 'occ db:convert-type', or see the {linkstart}documentation ↗{linkend}." : "要遷移到另一個數據庫，請使用指令工具：“ occ db：convert-type”，或參閱 {linkstart} 說明書↗{linkend}。",
