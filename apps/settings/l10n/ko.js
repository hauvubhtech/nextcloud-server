OC.L10N.register(
    ...
    "OPcache is configured to remove code comments. With OPcache enabled, <code>opcache.save_comments=1</code> must be set for xD Drive to function." : "OPcache가 code comment를 삭제하도록 설정되어 있습니다. xD Drive의 원활한 작동을 위해 OPcahce 활성화 시 <code>opcache.save_comments=1</code>을 설정해야 합니다.",
    "xD Drive is not allowed to use the OPcache API. With OPcache enabled, it is highly recommended to include all xD Drive directories with <code>opcache.restrict_api</code> or unset this setting to disable OPcache API restrictions, to prevent errors during xD Drive core or app upgrades." : "xD Drive가 OPcache API를 사용하도록 허용되지 않았습니다. xD Drive core나 앱 업그레이드 중 에러 발생을 방지하기 위해, OPcache 활성화 시 모든 xD Drive 디렉토리를 <code>opcahce.restrict_api</code>에 포함하거나 해당 설정을 해제하여 OPcahce API 제한을 비활성화 하는 것을 권장합니다.",
    ...
    "Check the security of your xD Drive over <a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">our security scan ↗</a>." : "<a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">우리의 보안 검사 ↗</a>에서 이 xD Drive의 보안을 점검하세요.",
    "Version" : "버전",
    ...
    "Reasons to use xD Drive in your organization" : "당신의 조직에서 xD Drive를 사용해야 할 이유",
    ...
    "If you use third party applications to connect to xD Drive, please make sure to create and configure an app password for each before enabling second factor authentication." : "xD Drive에 연결하기 위해 서드파티 애플리케이션을 사용할 경우 2차 인증을 활성화하기 앞서 각 애플리케이션에 대한 앱 암호를 생성, 설정하십시오.",
