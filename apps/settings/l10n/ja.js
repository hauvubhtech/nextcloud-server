OC.L10N.register(
    ...
    "OPcache is configured to remove code comments. With OPcache enabled, <code>opcache.save_comments=1</code> must be set for xD Drive to function." : "OPcacheがコードのコメントを削除するように設定されています。xD Driveを正しく動作させるため、OPcacheが有効の場合は、<code>opcache.save_comments=1</code>を設定する必要があります。",
    "xD Drive is not allowed to use the OPcache API. With OPcache enabled, it is highly recommended to include all xD Drive directories with <code>opcache.restrict_api</code> or unset this setting to disable OPcache API restrictions, to prevent errors during xD Drive core or app upgrades." : "xD DriveはOPcache APIを使用する事が許可されていません。OPcacheが有効の場合、xD Driveコアまたはアプリのアップグレード時にエラーを回避するため、全てのxD Driveディレクトリーに対して、<code>opcache.restrict_api</code>を含めるか、API制限を無効にするため、この設定を未設定にする事を強く推奨します。",
    "xD Drive is not allowed to use the OPcache API. It is highly recommended to include all xD Drive directories with <code>opcache.restrict_api</code> or unset this setting to disable OPcache API restrictions, to prevent errors during xD Drive core or app upgrades." : "xD DriveはOPcache APIを使用する事が許可されていません。xD Driveコアまたはアプリのアップグレード時にエラーを回避するため、全てのxD Driveディレクトリーに対して、<code>opcache.restrict_api</code>を含めるか、API制限を無効にするため、この設定を未設定にする事を強く推奨します。",
    ...
    "There are some user imported SSL certificates present, that are not used anymore with xD Drive 21. They can be imported on the command line via \"occ security:certificates:import\" command. Their paths inside the data directory are shown below." : "ユーザーがインポートしたSSL証明書がいくつか存在しますが、xD Drive 21ではもう使用されていません。これらの証明書は、\"occ security:certificates:import\" コマンドにより、コマンドラインでインポートすることができます。これらの証明書のデータディレクトリ内のパスは以下のとおりです。",
    ...
    "MariaDB version \"%s\" is used. xD Drive 21 and higher do not support this version and require MariaDB 10.2 or higher." : "MariaDB のバージョン\"%s\" が使われています。xD Drive 21以降ではこのバージョンのサポートは終了し、MariaDB 10.2 以降のバージョンが必要になります。",
    "MySQL version \"%s\" is used. xD Drive 21 and higher do not support this version and require MySQL 8.0 or MariaDB 10.2 or higher." : "MySQLのバージョン \"%s\" が使用されています。xD Drive 21以降ではこのバージョンのサポートは終了し、MySQL 8.0またはMariaDB 10.2以上が必要となります。",
    "PostgreSQL version \"%s\" is used. xD Drive 21 and higher do not support this version and require PostgreSQL 9.6 or higher." : "PostgreSQLのバージョン \"%s\" を使用しています。xD Drive 21以降ではこのバージョンのサポートは終了し、PostgreSQL 9.6以降が必要となります。",
    ...
    "xD Drive settings" : "xD Drive の設定",
    ...
    "Remove" : "削除",
    "Disable" : "無効にする",
    "This app has no minimum xD Drive version assigned. This will be an error in the future." : "このアプリは xD Drive の最小バージョンが指定されていません．将来、エラーが発生する可能性があります.",
    "This app has no maximum xD Drive version assigned. This will be an error in the future." : "このアプリは xD Drive バージョンの上限が指定されていません．将来、エラーが発生する可能性があります.",
    ...
    "This app is supported via your current xD Drive subscription." : "このアプリは、現行のxD Driveサブスクリプションでサポートされています。",
    "Supported" : "対応しています",
    ...
    "Encryption alone does not guarantee security of the system. Please see documentation for more information about how the encryption app works, and the supported use cases." : "暗号化だけで、システムのセキュリティが保証されるわけではありません。暗号化アプリがどのように動作するかの詳細な情報や、サポートされているユースケースについては、xD Drive のドキュメントを参照してください。",
    ...
    "In case of lost device or exiting the organization, this can remotely wipe the xD Drive data from all devices associated with {userid}. Only works if the devices are connected to the internet." : "デバイスを紛失した場合や、組織外に出た場合、{userid}に関連付けられたすべてのデバイスからリモートで xD Drive のデータを消去することができます。デバイスがインターネットに接続されている場合にのみ機能します。",
    ...
    "This app is not marked as compatible with your xD Drive version. If you continue you will still be able to install the app. Note that the app might not work as expected." : "このアプリはご利用中のxD Driveのバージョンと互換性があるとしてマークされていません。 続行しても、アプリをインストールすることはできます。 アプリが期待通りに動作しない可能性があることに注意してください。",
    "Never" : "なし",
    ...
    "Send email" : "メールを送信",
    "It's important for the security and performance of your instance that everything is configured correctly. To help you with that we are doing some automatic checks. Please see the linked documentation for more information." : "サーバーのセキュリティとパフォーマンスにとって重要なことは、すべてが正確に設定されていることです。あなたの助けとなるよう、xD Driveでは一部の自動チェックを行っています。詳細な情報は、リンク先のドキュメントを参照してください。",
    ...
    "Check the security of your xD Drive over <a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">our security scan ↗</a>." : "xD Driveのセキュリティは、<a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">セキュリティスキャン ↗</a>でチェックしてください。",
    "Version" : "バージョン",
    ...
    "Reasons to use xD Drive in your organization" : "あなたの組織でxD Driveを使うべきこれだけの理由",
    "Developed by the {communityopen}xD Drive community{linkclose}, the {githubopen}source code{linkclose} is licensed under the {licenseopen}AGPL{linkclose}." : "{communityopen} xD Driveコミュニティ{linkclose}によって開発された{githubopen}ソースコード{linkclose}は、{licenseopen} AGPL {linkclose}で許諾されています。",
    ...
    "This community release of xD Drive is unsupported and instant notifications are unavailable." : "このコミュニティリリースのxD Driveは未サポートです。インスタント通知も利用できません。",
    ...
    "If you use third party applications to connect to xD Drive, please make sure to create and configure an app password for each before enabling second factor authentication." : "サードパーティのアプリケーションを使用してxD Driveに接続する場合は、二要素認証を有効にする前に必ずそれぞれのアプリパスワードを作成して設定してください。",
