OC.L10N.register(
    ...
    "Could not determine if TLS version of cURL is outdated or not because an error happened during the HTTPS request against https://drive.xd-twin.io Please check the xD Drive log file for more details." : "不能确定 cURL 的 TLS 版本是否已过时，因为在 HTTPS 请求 https://drive.xd-twin.io 时发生了错误。请检查 nextcloud 日志文件以获取更多信息。",
    ...
    "OPcache is configured to remove code comments. With OPcache enabled, <code>opcache.save_comments=1</code> must be set for xD Drive to function." : "OPcache 被设置为移除代码注释。当启用 OPcache 后，必须设置 <code>opcache.save_comments=1</code>，才能让 xD Drive 工作。",
    "xD Drive is not allowed to use the OPcache API. With OPcache enabled, it is highly recommended to include all xD Drive directories with <code>opcache.restrict_api</code> or unset this setting to disable OPcache API restrictions, to prevent errors during xD Drive core or app upgrades." : "xD Drive 不被允许使用 OPcache API。启用 OPcache 后，强烈建议使用 <code>opcache.restrict_api</code> 包含所有 xD Drive 目录，或取消该设置以禁用 OPcache API 限制，以避免 xD Drive 核心或应用程序升级期间发生错误。",
    "xD Drive is not allowed to use the OPcache API. It is highly recommended to include all xD Drive directories with <code>opcache.restrict_api</code> or unset this setting to disable OPcache API restrictions, to prevent errors during xD Drive core or app upgrades." : "xD Drive 不被允许使用 OPcache API。强烈建议使用 <code>opcache.restrict_api</code> 包含所有 xD Drive 目录，或取消该设置以禁用 OPcache API 限制，以避免 xD Drive 核心或应用程序升级期间发生错误。",
    ...
    "MariaDB version \"%s\" is used. xD Drive 21 and higher do not support this version and require MariaDB 10.2 or higher." : "正在使用 MariaDB \"%s\" 版本。xD Drive 21 及更新版本不再支持此版本，需要 MariaDB 10.2 或更高版本。",
    "MySQL version \"%s\" is used. xD Drive 21 and higher do not support this version and require MySQL 8.0 or MariaDB 10.2 or higher." : "正在使用 MySQL 版本 \"%s\"。xD Drive 21 及更新版本不再支持此版本，需要 MySQL 8.0 或 MariaDB 10.2 或更高版本。",
    "PostgreSQL version \"%s\" is used. xD Drive 21 and higher do not support this version and require PostgreSQL 9.6 or higher." : "正在使用 PostgreSQL \"%s\" 版本。xD Drive 21 及更新版本不再支持此版本，需要 PostgreSQL 9.6 或更高版本。",
    ...
    "Remove" : "移除",
    "Disable" : "禁用",
    "This app has no minimum xD Drive version assigned. This will be an error in the future." : "该应用没有指定支持的 xD Drive 最低版本。可能会在将来出现问题。",
    "This app has no maximum xD Drive version assigned. This will be an error in the future." : "该应用没有指定支持的 xD Drive 最高版本。可能会在将来出现问题。",
    ...
    "This app is supported via your current xD Drive subscription." : "根据您的 xD Drive 订阅，此应用受到支持。",
    "Supported" : "受支持",
    ...
    "In case of lost device or exiting the organization, this can remotely wipe the xD Drive data from all devices associated with {userid}. Only works if the devices are connected to the internet." : "如果设备丢失或退出组织，这可以从与 {userid} 关联的所有设备中远程擦除 xD Drive 数据。 仅当设备连接到互联网时才有效。",
    ...
    "This app is not marked as compatible with your xD Drive version. If you continue you will still be able to install the app. Note that the app might not work as expected." : "此应用被标记为与您的 xD Drive 版本不兼容。如果您要继续，您仍可安装此应用。请注意此应用可能不会正常工作。",
    "Never" : "从不",
    ...
    "Check the security of your xD Drive over <a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">our security scan ↗</a>." : "通过<a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">我们的安全检查 ↗</a>检查您 xD Drive 的安全",
    "Version" : "版本",
    ...
    "Reasons to use xD Drive in your organization" : "在您的组织中使用 xD Drive 的原因",
    "Developed by the {communityopen}xD Drive community{linkclose}, the {githubopen}source code{linkclose} is licensed under the {licenseopen}AGPL{linkclose}." : "由 {communityopen}xD Drive community{linkclose} 开发，{githubopen}源代码{linkclose} 基于 {licenseopen}AGPL{linkclose} 许可协议。",
    ...
    "This community release of xD Drive is unsupported and instant notifications are unavailable." : "xD Drive 的这个社区版本不受支持，即时通知不可用。",
    "Use a second factor besides your password to increase security for your account." : "使用除您密码之外的第二种方法来增强您账号的安全性。",
    "If you use third party applications to connect to xD Drive, please make sure to create and configure an app password for each before enabling second factor authentication." : "如果您使用第三方应用程序连接到 xD Drive，请确保在启用第二因素身份验证之前为每个应用程序创建和配置应用程序密码。",
    ...
    "Could not determine if TLS version of cURL is outdated or not because an error happened during the HTTPS request against https://drive.xd-twin.io Please check the nextcloud log file for more details." : "不能确定 cURL 的 TLS 版本是否已过时，因为在HTTPS请求 https://drive.xd-twin.io 时发生了错误。请检查 nextcloud 日志文件以了解详情。",
