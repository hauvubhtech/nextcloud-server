OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "这个应用能够让管理员们将一个基于LDAP的用户目录与Nextcloud连接起来",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "本应用启用功能使管理员可将 xD Drive 与基于 LDAP 的用户目录连接，进行身份验证以及提供用户、群组和用户属性。管理员可配置此程序通过 LDAP 接口连接至一个或多个 LDAP 或 Active Directory 目录。通过适当的查询和筛选，可将以下属性从目录导入 xD Drive: 用户磁盘配额、电子邮箱、头像、所属群组以及更多。\n\n用户可以用其LDAP或AD中的身份登录 xD Drive, 并根据 LDAP 或 AD 服务的身份验证获得访问权限。Nextcloud 仅使用身份验证随后为用户身份使用会话，但不会储存LDAP或AD的密码。您可从 LDAP 用户及群组后台文档中获取更多信息。",
    ...
    "Usernames are used to store and assign metadata. In order to precisely identify and recognize users, each LDAP user will have an internal username. This requires a mapping from username to LDAP user. The created username is mapped to the UUID of the LDAP user. Additionally the DN is cached as well to reduce LDAP interaction, but it is not used for identification. If the DN changes, the changes will be found. The internal username is used all over. Clearing the mappings will have leftovers everywhere. Clearing the mappings is not configuration sensitive, it affects all LDAP configurations! Never clear the mappings in a production environment, only in a testing or experimental stage." : "用户名用于存储和分配元数据。为了精确的区分和识别用户，每个 LDAP 用户都会有一个内部的用户名。这要求建立一个用户名到 LDAP 用户的映射。创建的用户名会被映射到 LDAP 用户的 UUID。另外为了节省 LDAP 连接开销，DN 会被缓存，但不会用于识别。如果 DN 变了，这些变化会被识别到。在 xD Drive 各个页面会使用内部用户名。清空映射会造成系统里面有大量的残留信息。清空映射会影响所有的 LDAP 配置，而不仅仅是当前配置。不要在生产环境里面应用清空映射，请仅用于测试环境或者早期验证步骤。",
